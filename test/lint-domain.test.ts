...
describe('lintDomain', () => {
  ...
  describe('followTomlPointers', () => {
    it('fetches and lints valid toml pointers', async () => {
      ...
      const impl = (async (url: string | URL) => {
        if (String(url).endsWith('stellar.toml')) {
          return new Response(mainToml, { status: 200, headers: { 'content-type': 'text/plain', 'access-control-allow-origin': '*' } });
        } else {
          return new Response(assetToml, { status: 200, headers: { 'content-type': 'text/plain', 'access-control-allow-origin': '*' } });
        }
      }) as unknown as typeof fetch;
    ...
    it('gracefully handles missing toml pointers as warnings', async () => {
      const mainToml = [
        '[[CURRENCIES]]',
        'toml="https://example.com/broken.toml"',
      ].join('\n');
