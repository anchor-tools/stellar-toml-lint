...
# 1. Add `import { watch } from 'node:fs';`
content = content.replace("import { readFile, writeFile } from 'node:fs/promises';", "import { readFile, writeFile } from 'node:fs/promises';\nimport { watch } from 'node:fs';")

# 2. Add `watch?: boolean;` to `Cli` interface
content = content.replace("  strict: boolean;\n", "  strict: boolean;\n  watch?: boolean;\n")

# 3. Add `--watch` to `parseArgs`
...
# 4. Wrap main body in runLint
start_match = re.search(r'const color = cli\.color \?\? shouldUseColor\(\);\n', content)
start_idx = start_match.end()

end_match = re.search(r'return verdict\(results, { strict, maxWarnings }\) \? 0 : 1;\n}', content)
