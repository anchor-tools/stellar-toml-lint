...
# Find "const color = cli.color ?? shouldUseColor();\n"
start_match = re.search(r'const color = cli\.color \?\? shouldUseColor\(\);\n', content)
start_idx = start_match.end()

# Find "return verdict(results, cli) ? 0 : 1;\n}"
end_match = re.search(r'return verdict\(results, cli\) \? 0 : 1;\n}', content)
end_idx = end_match.end() - 2 # exclude the closing brace of main
